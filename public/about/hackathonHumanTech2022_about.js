...
function preload() {
  ...
  oswaldMedium = loadFont('fonts/Oswald-Medium.ttf');
  robotoCondBold = loadFont('fonts/RobotoCondensed-Bold.ttf');
  robotoCondRegular = loadFont('fonts/RobotoCondensed-Regular.ttf');

  warexpo = loadImage('logo/warexpo.png');
  ovh= loadImage('logo/ovh.png');
  humantech = loadImage('logo/humantech.png');
  ...
  nearEarthObjectsBase = loadJSON("https://api.nasa.gov/neo/rest/v1/feed?start_date="+combineDate+"&end_date="+combineDate+"&api_key=wciCTc95fXs5eaAuOVLc20ds3rrdjIjlxrJJGoVU");
...
function setVar(todaysDate) {
  ...
  for (let i = 0; i <objectCount; i++) {
    let di = nearEarthObjectsBase.near_earth_objects[todaysDate][i].estimated_diameter.kilometers.estimated_diameter_max;
    diameters[i] = minDistBetweenObj/2 + minDistBetweenObj*di;
    hazardous[i] = nearEarthObjectsBase.near_earth_objects[todaysDate][i].is_potentially_hazardous_asteroid;
    velocitiesSec[i] = nearEarthObjectsBase.near_earth_objects[todaysDate][i].close_approach_data[0].relative_velocity.kilometers_per_second;
    velocitiesHour[i] = nearEarthObjectsBase.near_earth_objects[todaysDate][i].close_approach_data[0].relative_velocity.kilometers_per_hour;
    missDistances[i] = nearEarthObjectsBase.near_earth_objects[todaysDate][i].close_approach_data[0].miss_distance.kilometers;
    names[i] = nearEarthObjectsBase.near_earth_objects[todaysDate][i].name;
    colors[i] = color(30+velocitiesHour[i]%100, 100, 90);

    let xu = minDistBetweenObj + i*(width-minDistBetweenObj)/objectCount;
...
function draw() {
  ...
  for (let i = 0; i <objectCount; i++) {//objects in proximity of Earth
    ...
    for (let j = 0; j < distV; j += 40) {//colors under the objects
  ...
  for (let i = 0; i <objectCount; i++) {//shapes and descriptions of the objects
  ...
  textFont(robotoCondBold);
  textSize(22);
  text("Jakie relacje tworzymy z technolgią? Czy technologia jest problemem czy rozwiązaniem? Trzy prace, które można zobaczyć na tych ekranach stanowią próbę odpowiedzi na te pytania. Są to interaktywne wizualizacje stworzone przez zespoły, które weszły do finału Hackathonu Creative Coding w ramach konferencji Human-Tech Summit 2022, zorganizowanej przez Uniwersytet SWPS.", 40, height-360, 800, 800);

  textSize(16);
  text("Animacja powyżej jest ilustracją danych udostępnianych przez NASA dotyczących meteorytów przelatujących aktualnie blisko Ziemi.", 40, height-200, 800, 800);
  ...
  if (timeCount <= 0) { //load first visualisation
