...
function createqr() {
  let nazwa =
    "https://human-tech-hackaton-22.vercel.app/404/index2.html?screen=" +
    String(id) +
    "&date=" +
    String(data);
...
function refresh() {
  ...
  if (
    data != 956259420000 &&
    Math.floor((Date.now() - Number(data)) / 1000) > 89
  ) {
    id = futureid;
    data = 956259420000;
    restart();
...
function getVote() {
  ...
  $.when(
    ...
    document.getElementById("pollResult").innerHTML =
      "Tak: " + String(yes) + "%" + "\nNie: " + String(no) + "%";
    document.getElementById("tekstAn3").innerHTML =
      "liczba głosów: " + String(suma);
  });
}
